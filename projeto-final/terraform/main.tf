...
resource "aws_secretsmanager_secret" "argocd_cluster_secret_dev" {
  name = "argocd-cluster-secret-dev-lab"
}
resource "aws_secretsmanager_secret" "argocd_cluster_secret_prod" {
  name = "argocd-cluster-secret-prod-lab"
}
####################### Secret Version #########################

resource "aws_secretsmanager_secret_version" "argocd_cluster_dev_secret_version" {
  secret_id = aws_secretsmanager_secret.argocd_cluster_secret_dev.id
...
resource "aws_secretsmanager_secret_version" "argocd_cluster_prod_secret_version" {
  secret_id = aws_secretsmanager_secret.argocd_cluster_secret_prod.id
  secret_string = jsonencode({
    config = {
